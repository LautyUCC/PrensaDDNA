...
?>
<section class="home-section home-programs" aria-labelledby="programs-title" data-carousel>
	<div class="container container--wide section-frame">
		<header class="section-frame__header"><h2 id="programs-title">Programas</h2></header>
		...
		<div class="carousel carousel--programs<?php echo $programs->post_count <= 2 ? ' carousel--sparse' : ''; ?>" id="programs-carousel" data-carousel-track tabindex="0" role="group" aria-roledescription="<?php esc_attr_e( 'carrusel', 'ddna-theme' ); ?>" aria-label="<?php esc_attr_e( 'Programas destacados', 'ddna-theme' ); ?>">
			<?php while ( $programs->have_posts() ) : $programs->the_post(); get_template_part( 'template-parts/components/program-card' ); endwhile; ?>
		</div>
	</div>
</section>
