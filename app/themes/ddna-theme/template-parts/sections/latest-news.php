...
?>
<section class="home-section home-news" aria-labelledby="news-title" data-carousel>
	<div class="container container--wide section-frame">
		<header class="section-frame__header"><h2 id="news-title">Novedades</h2></header>
		...
		<div class="carousel carousel--news<?php echo $latest_news->post_count <= 3 ? ' carousel--sparse' : ''; ?>" id="news-carousel" data-carousel-track tabindex="0" role="group" aria-roledescription="<?php esc_attr_e( 'carrusel', 'ddna-theme' ); ?>" aria-label="<?php esc_attr_e( 'Últimas novedades', 'ddna-theme' ); ?>">
			<?php while ( $latest_news->have_posts() ) : $latest_news->the_post(); get_template_part( 'template-parts/components/news-card' ); endwhile; ?>
		</div>
	</div>
</section>
